...
import 'package:first_project_flutter/models/becaon_model.dart';
import 'dart:math';
...
class DataService{
  ...
  Future<List<String>> getAllBeaconInfo() async{
    ...
    for (var i in body){
      beaonlist.add(BeaconModel(beaconId: i['BeaconId'], locationOfBeacon: i['LocationOfBeacon']));
  ...
  Future sendLocationInfoToService(Map<String,List> locationInfo) async{
    ...
    for (var k in locationInfo.values) {
      ...
      await http.post(url, body: json.encode({"beaconId": beaconId,"patientId": uuid,"distance": distance}));
    }
  }
